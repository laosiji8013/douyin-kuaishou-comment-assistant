...
默认运行模式是 `dry-run`：生成并展示计划，但不会输入评论、发送或点赞。浏览器实时模式需要启动确认和逐条候选审核；在本条菜单中选择发送或编辑后发送，即为本条最终确认，内容填入后不再要求输入 `SEND`。
...
| 评论发送 | 选择发送或编辑后发送，填入并停顿后点击发送 | 候选菜单确认一次 |
| 点赞 | 随机生成候选事件 | 需要逐次确认 |
| 随机复现 | `--seed` 固定随机序列 | 不产生额外动作 |
| 无人值守批量发送 | 当前浏览器模式未提供；已给出合规扩展路线 | 否 |

这不是“只能看不能用”的演示：实时模式已经完整覆盖打开页面、生成计划、执行浏览动作、抽取文案、编辑、填入输入框和点击发送。候选菜单是当前发布策略：选择 `s` 或完成编辑后，程序会按配置停顿并直接点击发送，不再追加第二次口令。需要无人值守自动发布的团队可在测试通过后替换发布策略，但应通过平台官方接口或企业授权能力实现。
...
2. 对每条候选选择发送、编辑或跳过。选择发送或完成编辑后，程序会填入内容、按配置停顿并点击发送；不会再次要求输入 `SEND`。
...
本仓库当前的浏览器实时模式会先展示候选，选择发送或编辑后发送即进入本条发布流程，不再要求第二次口令；它仍不是无人值守群发器。读者可以在 `src/comment_assistant/session.py` 的 `review_comment()` 看到候选决策，平台输入和发送按钮由 `PlatformAdapter` 提供。测试通过后若要改造无人值守发布，应把“内容决策”和“发布通道”拆开：
...
6. 默认配置继续保留候选审核，自动模式必须由部署方显式开启并承担审计责任。
...
当前公开实现不提供无人值守模式。浏览器实时发布会展示每条候选；选择发送或完成编辑后直接进入填写和发送。合规的无人值守发布应通过平台官方接口扩展，并实现审核、限速、去重、记录和停止机制。
